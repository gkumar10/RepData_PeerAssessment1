...
```{r echo=TRUE}
df_no_na <- na.omit(df)
df_steps_by_day <- aggregate(df_no_na$steps, by=list(df_no_na$date), FUN=sum)
names(df_steps_by_day) <- c("date", "steps")
hist(df_steps_by_day$steps, 
...
```{r echo=TRUE}
meanvalue <- mean(df_steps_by_day$steps)
medianvalue <- median(df_steps_by_day$steps)
```
...
```{r echo=TRUE}
new_df_steps_by_day <- aggregate(df_na_filled$steps, by=list(df_na_filled$date), FUN=sum)
names(new_df_steps_by_day) <- c("date", "steps")
hist(new_df_steps_by_day$steps, 
     main="Total Steps Each Day", 
     col="grey", 
     xlab="Number of steps",
     breaks=10)

new_meanvalue <- mean(new_df_steps_by_day$steps)
new_medianvalue <- median(new_df_steps_by_day$steps)
```

With missing values imputed, mean is `r as.integer(new_meanvalue)`

With missing value imputed, median is `r as.integer(new_medianvalue)`
...
We notice higher frequency counts in the potion of histogram around the mean value of 10766.
